...
def fasttext(train_features, test_features, real_features, vector_size, min_count, class_mode, represent_mode, key_tag):
    # 数据格式转换
    ...
    # 训练模型
    model = gensim.models.fasttext.FastText(sentences=MyCorpus(train_features, key_tag), vector_size=vector_size,
                                            min_count=min_count, min_n=3, max_n=6, bucket=2000000)
    # 保存模型
    save_model(model, class_mode, represent_mode, key_tag)
    fw = open(f"../data/models/fasttext/{class_mode}-{represent_mode}-{key_tag}_error.log", "w")
    ...
    real_features = sentence2vec(model, real_features, vector_size, fw, represent_mode, flag="train_test_real set")
...
def load_model(mode, key_tag):
    fr = open(f"../data/models/fasttext/{mode}-{key_tag}.txt", "r")
...
def save_model(model, class_mode, represent_mode, key_tag):
    ...
    fw = open(f"../data/models/fasttext/{class_mode}-{represent_mode}-{key_tag}.txt", "w")
    with tempfile.NamedTemporaryFile(prefix=file_name, delete=False, dir='../data/models/fasttext') as tmp:
