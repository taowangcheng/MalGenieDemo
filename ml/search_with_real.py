...
def ml(x_train, y_train, x_test, y_test, x_real, y_real, class_mode, represent_mode, mode):
    ...
    gridsearch(x_train, y_train, x_test, y_test, x_real, y_real, target_names[mode],
               class_mode, represent_mode)
    np.savez('../data/result_search.npz', mal_dict=result_dict)

# 机器学习模型超参数搜索函数
def gridsearch(x_train, y_train, x_test, y_test, x_real, y_real, target, class_mode, represent_mode):
    clf_mnb = Pipeline([('nb', MultinomialNB())])
    parameters_mnb = {
        'nb__alpha': np.arange(0.1, 4, 0.1),
    }
    ...
    parameters_dt = {
        ...
        'DTree__max_depth': np.arange(1, 10, 1),
    ...
    for clf_single, parameters, name in \
        ...
        print("GridSearching: ")
        print(name)
        gs = GridSearchCV(clf_single, parameters, verbose=2, refit=True, cv=10, n_jobs=10)
